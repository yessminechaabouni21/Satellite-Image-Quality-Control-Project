...
# Full feature set (16 cols) — used by default
FEATURE_COLS = [
...
# Reduced set — ONLY the features that passed diagnose_separation.py with
# p<0.10. Diluting 2 real signals across 14 noise features is why the full
# model underperforms a simple threshold; this set fixes that directly.
# Update this list yourself if you re-run diagnose_separation.py and get
# different significant features.
VALIDATED_FEATURE_COLS = [
    "BlurFilter__dn_range",        # p=0.001, Cohen's d=1.72
    "TOAScalingFilter__max_dn",    # p=0.081, Cohen's d=0.83
    "BlurFilter__laplacian_variance",  # p=0.16, Cohen's d=-0.67 (next best)
]
...
def train(df, X, feat_cols, source_filter=None):
    # Train ONLY on clean scenes (label==0), optionally restricted to one source
    clean_mask = df["label"] == 0
    if source_filter:
        clean_mask &= (df["source"] == source_filter)
...
def main():
    ...
    ap.add_argument("--source", default=None,
                    help="Train only on this source's clean scenes "
                        "(e.g. 'esa_ref'). Default: all sources.")
    ap.add_argument("--features", choices=["all", "validated"], default="all",
                    help="'all' = full 16-feature set. "
                        ...
                        "diagnose_separation.py (BlurFilter__dn_range, "
                        "TOAScalingFilter__max_dn, BlurFilter__laplacian_variance). "
                        "Use 'validated' to stop diluting real signal with noise.")
    args = ap.parse_args()
    ...
    model, scaler = train(df, X, feat_cols, source_filter=args.source)
    ...
    model_path  = REPORT_DIR / f"isolation_forest{suffix}.pkl"
    scaler_path = REPORT_DIR / f"if_scaler{suffix}.pkl"
    cols_path   = REPORT_DIR / f"if_feature_cols{suffix}.json"
    ...
    if metrics:
        with open(REPORT_DIR / f"if_metrics{suffix}.json", "w") as f:
            json.dump(metrics, f, indent=2)

    print(f"\nNext: python -m src.ml.train_random_forest "
          f"--source {args.source or '<none>'} --features {args.features}")
